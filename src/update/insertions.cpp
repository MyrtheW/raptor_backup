...
void insert_ubs(update_arguments const & arguments,
...
std::tuple <uint64_t, uint64_t>  find_empty_bin_idx(raptor_index<index_structure::hibf> & index, size_t ibf_idx, size_t number_of_bins){
    ...
    } // if the whole loop is run through, i.e. bin_idx = ibf_bin_count, no appropiate empty bin is found and the bin idx will be the size of the IBF.
    ...
    if (bin_idx == ibf_bin_count){// then there is no empty bin. Resize IBF .  or ibf.bin_count()
        ...
        // todo should best be a multiple of 64. Make sure that when calling void increase_bin_number_to(bin_count const new_bins_), it increases.
        index.ibf().resize_ibf(ibf_idx, new_ibf_bin_count);

        //index.ibf().resize_ibf_filename_positions(ibf_idx, new_ibf_bin_count);

        // Check if a resize is needed for any of the other filename related datastructures.
